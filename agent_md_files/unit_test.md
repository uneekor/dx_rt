...
### 핵심 원칙 (모든 테스트 작성 시 준수)

**1. 품질과 Coverage는 함께 달성**
```
❌ 잘못된 목표: "Line coverage 80% 달성" (숫자만)
❌ 잘못된 목표: "중요 경로만 검증" (coverage 무시)
✅ 올바른 목표: "모든 중요 경로를 검증하면서 Coverage 80% 이상 달성"
```

**달성 방법:**
- 중요 경로 먼저 검증 (품질 우선)
- Coverage 측정 → 미커버 분석
- 미커버 중 중요 경로 추가 검증 (품질 유지하며 coverage 상승)
- **억지 테스트로 숫자만 채우기 절대 금지**

**Coverage 목표:**
| 항목 | 목표 | 접근 |
|------|------|------|
| Lines | ≥80% | 모든 함수의 주요 경로 검증 |
| Functions | ≥90% | Public API 전체 + 중요 Private 검증 |
| Branches | ≥60% | 에러 처리, 조건 분기 검증 |

**2. 모든 테스트는 최소 1개 이상 Assertion 필수**
```cpp
❌ 금지: TEST_F(...) { func(); }  // assertion 없음
❌ 금지: TEST_F(...) { func(); SUCCEED(); }
✅ 필수: TEST_F(...) { int rc = func(); EXPECT_EQ(0, rc); }
```

**3. 의미 있는 테스트만 작성 (억지 테스트 금지)**
- ✅ 좋은 이유: "이 에러 경로를 검증해야 함" → 테스트 작성
- ❌ 나쁜 이유: "Coverage 80%에 2% 부족" → SUCCEED() 추가
- ❌ 나쁜 이유: "Branch coverage 올리려고" → Assertion 없는 테스트
- **판단 기준**: "이 테스트가 없으면 버그 발견 못할까?" YES면 작성, NO면 작성 안 함

**4. 테스트는 "무엇이 잘못되었는지" 알려줘야 함**
- 단순히 "통과/실패"가 아님
- 실패 시 어떤 값이 기대값과 달랐는지 명확히 표시
- 예: `EXPECT_EQ(expected, actual)` (actual 값이 메시지에 출력됨)

**5. 테스트 코드도 유지보수 대상**
- 중복 코드 제거 (Fixture 활용)
- 명확한 이름 (무엇을 검증하는지 이름만 봐도 알 수 있게)
- 간결한 로직 (테스트가 1000줄이면 뭔가 잘못됨)

---

### 2.1 AAA 패턴 (기본 원칙 — 모든 테스트 적용)

**모든 TEST_F 본문은 AAA 구조를 따른다.**
...
#### ❌ 3) Assertion 없는 테스트
```cpp
TEST_F(BadTest, JustCallFunction)
{
    handler_->Process();  // ← Assert 없음!
}

// ✅ GOOD: 최소 1개 이상 assertion
TEST_F(GoodTest, ProcessDoesNotCrash)
{
    EXPECT_NO_THROW(handler_->Process());
}
```
...
{
    // ❌ BAD: 불필요한 명시적 해제 (RAII 자동 처리)
    handler_.reset();
    buffer_.clear();
    
    // ✅ GOOD: RAII로 자동 해제됨. TearDown 생략 가능.
}
```
**원칙**: 파일 핸들, DB 연결 등 명시적 해제 필요한 경우만 TearDown 사용.

#### ❌ 5) Magic Number
```cpp
// ❌ BAD
TEST_F(BadTest, EncodeData)
{
    int rc = Encode(buffer_, 1024, 16, 4);  // ← 1024, 16, 4가 뭐지?
}

// ✅ GOOD
TEST_F(GoodTest, EncodeData)
{
    const uint32_t kBufferSize = 1024;
    const uint32_t kColumns = 16;
    const uint32_t kAlignUnit = 4;
    int rc = Encode(buffer_, kBufferSize, kColumns, kAlignUnit);
}
```

#### ❌ 6) Coverage Farming - 억지로 커버리지 올리기

**최악의 Anti-Pattern: 숫자만 올리고 품질은 안 올리는 테스트**

```cpp
// ❌ WORST: SUCCEED()만 있는 테스트 (최악)
TEST_F(BadTest, ErrorPathCoverage) {
    handler_->ProcessWithNull(nullptr);  // 코드 실행만
    SUCCEED();  // "크래시 안 나면 통과"
}

// ❌ BAD: Assertion 없이 코드만 실행
TEST_F(BadTest, TriggerErrorBranch) {
    handler_->InvalidOperation();  // Branch coverage +1
    // 아무 검증도 없음!
}

// ❌ BAD: EXPECT_NO_FATAL_FAILURE만 있음 (너무 약한 검증)
TEST_F(BadTest, ProcessResponse_NoCrash) {
    Response resp{};
    EXPECT_NO_FATAL_FAILURE(handler_->Process(resp));
    // 잘못된 결과 반환해도 통과
}

// ✅ GOOD: 실제 동작 검증
TEST_F(GoodTest, ProcessNullInput_ReturnsErrorCode) {
    int rc = handler_->ProcessWithNull(nullptr);
    
    EXPECT_EQ(-EINVAL, rc);                // 에러 코드 확인
    EXPECT_TRUE(handler_->HasError());     // 에러 상태 확인
    EXPECT_EQ(0, handler_->ProcessedCount());  // 부작용 없음 확인
}

// ✅ GOOD: "NoCrash" 테스트도 결과 검증 필수
TEST_F(GoodTest, ProcessUnregisteredRequest_ReturnsSuccess) {
    Response resp{};
    
    EXPECT_NO_THROW({
        int rc = handler_->Process(resp);
        EXPECT_EQ(0, rc);  // ← 결과값 검증 추가
    });
    
    EXPECT_TRUE(handler_->IsIdle());  // 상태 검증
}
```

**Coverage Farming이 왜 나쁜가:**

| 측면 | Coverage Farming | 진짜 테스트 |
|------|-----------------|-----------|
| Line coverage | ✅ 80% 달성 | ✅ 80% 달성 |
| Branch coverage | ✅ 60% 달성 | ✅ 60% 달성 |
| **버그 검출** | ❌ 0% | ✅ 90%+ |
| **회귀 방지** | ❌ 안 됨 | ✅ 됨 |
| **유지보수 비용** | ❌ 높음 (의미 없는 테스트) | ✅ 낮음 |
| **코드 리뷰** | ❌ "이 테스트가 뭘 검증?" | ✅ 명확 |

**원칙: 품질과 Coverage 둘 다 달성**
- ❌ 잘못: "Coverage 80% 달성" → SUCCEED() 남발로 숫자만 채움
- ❌ 잘못: "품질만 중요" → Coverage 무시, 많은 경로 미검증
- ✅ 올바름: "모든 중요 경로 검증 + Coverage 80%" → 억지 테스트 없이 달성

**SUCCEED() 사용 금지 원칙:**

| 상황 | 허용 여부 | 대신 사용할 것 |
|------|----------|---------------|
| 일반 unit test | ❌ **절대 금지** | 최소 1개 이상 assertion |
| "NoCrash" 테스트 | ❌ 금지 | `EXPECT_NO_THROW` + **결과 검증** |
| Branch coverage용 | ❌ **절대 금지** | 실제 동작 검증하는 테스트 |
| 컴파일 타임 테스트 | ⚠️ 극히 제한적 허용 | 템플릿 인스턴스화 확인만 |

**탐지 방법:**

```bash
# SUCCEED() 사용 테스트 찾기
grep -rn "SUCCEED()" test/unittest/*.cpp

# Assertion 없는 테스트 찾기 (수동 검토 필요)
grep -A 10 "TEST_F" test/unittest/*.cpp | grep -v "EXPECT_\|ASSERT_"
```

**Code Review 체크리스트:**
- [ ] 모든 TEST_F에 최소 1개 이상 assertion 있는가?
- [ ] SUCCEED()가 있다면 정당한 이유가 있는가?
- [ ] EXPECT_NO_FATAL_FAILURE 뒤에 결과 검증이 있는가?
- [ ] "이 테스트가 뭘 검증하는가?" 질문에 답할 수 있는가?
...
```cpp
// 패턴: TEST_F(FixtureName, MethodName_Scenario_ExpectedResult)

TEST_F(NpuHandlerTest, Encode_NullInput_ReturnsError)
TEST_F(NpuHandlerTest, Encode_ValidInput_Success)
TEST_F(NpuHandlerTest, Encode_Alignment4_ProducesCorrectSize)
TEST_F(NpuHandlerTest, Decode_AfterEncode_MatchesOriginal)  // round-trip

// 축약 가능 (명확한 경우)
TEST_F(NpuHandlerTest, EncodeNullInput)
TEST_F(NpuHandlerTest, EncodeSuccess)
```

**원칙**: 테스트 이름만 보고 "무엇을 테스트하는지" 파악 가능해야 함.

---

### 2.7 Coverage 중심 체크리스트
...
**목표: 품질을 유지하면서 Coverage 80% 이상 달성**
...
Step 1: 중요 경로 우선 검증
    ...
    - 에러 처리 경로 검증
    ↓
Step 2: Coverage 측정 (섹션 4)
    - Line, Function, Branch coverage 확인
    ↓
Step 3: 미커버 분석
    - Line: gcov ##### 추출 + FNDA:0 확인
    - Branch: `gcov -b` → "branch never executed" 추출
    ↓
Step 4: 미커버 분류 (중요도 기준)
    A. **높은 우선순위 (반드시 테스트 추가)**:
       - Public API 미커버
       - 에러 처리 경로 (Mock 실패 반환)
       - 복합 조건식 미테스트 조합
       - 사용자 입력 검증 로직
    ...
    B. **중간 우선순위 (테스트 추가 권장)**:
       - Private 함수 중 복잡한 로직
       - 삼항 연산자, switch/case 미커버 분기
       - Boundary check
       
    C. **제외 또는 낮은 우선순위**:
       - catch(bad_alloc) 방어 코드 → LCOV_EXCL_BR_LINE
       - 수학적 도달 불가 bounds check → LCOV_EXCL_BR
       - #ifdef 다른 빌드 구성 → 해당 빌드에서 측정
       - HW/driver 의존 경로 → 섹션 6 기록
    ↓
Step 5: 테스트 추가
    - A: _error_test.cpp 또는 _edge_test.cpp에 추가
    - B: 우선순위에 따라 추가
    - C: 코드에 LCOV_EXCL 마킹 또는 리포트
    - **주의: SUCCEED() 금지, 모든 테스트에 assertion 필수**
    ↓
Step 6: 재측정 및 검증
    - ./build.sh → 테스트 실행
    - Coverage 재측정 → 개선 확인
    - 목표 미달성 시 Step 3으로 반복
    ↓
Step 7: 품질 검증
    - Code Review 체크리스트 확인 (섹션 2.5 #6)
    - 모든 테스트에 적절한 assertion 있는지 확인
    - SUCCEED() 사용 테스트 제거
```

**Coverage 80% 달성 못한 경우:**
1. 미커버 중 A, B 카테고리 재검토
2. Dead code 확인 (섹션 2.10) → 삭제
3. Testability 개선 (섹션 6 Refactoring 권고 참고)
4. **절대 금지**: 억지로 SUCCEED() 테스트 추가
